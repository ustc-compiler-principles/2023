...
我们提供了 `test_syntax.sh` 脚本进行快速批量测试。该脚本的第一个参数可以是 `easy`、 `normal`、 `hard` 以及 `testcases_general`，并且有第二个可选参数，用于批量 `diff` 和助教提供的标准参考结果进行比较。脚本运行后会将生成结果放在 `tests/1-parser/output_student` 文件夹里，而助教的参考输出则在 `tests/1-parser/output_standard` 中。
